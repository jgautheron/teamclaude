...
| `codexWarmupModel` | Model for a Codex account's [keep-warm](quota.md#keep-warm) run (`codex exec -m …`). Unset by default, which uses Codex's own default model |
...
| `accounts[].maxUsage` | Hard per-account usage cap: a number, or a per-bucket table like `{ "unified7d": 0.6, "unified7dFable": 0.8 }` (same keys as `switchThreshold`, plus `unified30d` and `codex:<slug>` for a Codex account's month and model families — see [Codex windows](quota.md#codex-windows); `default` covers unlisted buckets, anything else is uncapped). At the cap the account receives **no** requests — rotation skips it, the all-exhausted revalidation probe skips it, and a pin gets the exhausted answer. Model-scoped like the thresholds, applied live on reload. See [Per-account usage caps](quota.md#per-account-usage-caps) |
